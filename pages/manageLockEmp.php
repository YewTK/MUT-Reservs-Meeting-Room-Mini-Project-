...
$_SESSION['pageName'] = "manageLockEmp";
...
if (isset($_POST['search'])) {
 ...
 CASE WHEN e.AccountStatusID = 2 THEN 1 ELSE 0 END AS IS_LOCKED
 FROM Employee e 
 JOIN Department d ON e.DepartmentID = d.DepartmentID
 JOIN Position p ON e.PositionID = p.PositionID
 JOIN AccountStatus a ON e.AccountStatusID = a.AccountStatusID ";
...
if (isset($_POST['btn-unlock'])) {
    ...
    if ($empID != $_SESSION['EmployeeID']) {
        // เปลี่ยนคำสั่ง SQL เพื่ออัปเดตค่า ACCOUNTSTATUSID
        $UpdateQuery = "UPDATE Employee SET ACCOUNTSTATUSID = 1, COUNTLOCK = 0 WHERE EmployeeID = :empID";
        $isUnlock = $db->nonQuery($UpdateQuery, [':empID' => $empID]);
        echo json_encode([
            'success' => $isUnlock,
            'message' => $isUnlock ? "ปลดล็อกพนักงานเรียบร้อยแล้ว" : "ไม่สามารถลบปลดล็อกได้"
        ]);
    } else {
        echo json_encode(['success' => false, 'message' => "คุณไม่สามารถปลดล็อกตัวเองได้"]);
    }
    exit();
}
?>
...
<body>
    <div class="f-1">
        ...
        <div class="container">
            ...
            <div class="report-container-table table-responsive">
                            ...
                            <?php foreach ($employees as $employee): ?>
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <?php echo htmlspecialchars($employee['ACCOUNTSTATUSID'] == 2 ? 'Lock' : 'Unlock'); ?>
                                    </td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center align-items-center">
                                            <?php if ($employee['ACCOUNTSTATUSID'] == 2): // If employee is locked ?>
                                                <button type="button" class="btn btn-danger btn-unlock"
                ...
                <script>
                    $(document).ready(function () {
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    try {
                                        ...
                                        const employees = typeof response === 'object' ?
                                            response :
                                            JSON.parse(response);
                                ...
                                <tr>
                                    ...
                                    <td>${employee.ACCOUNTSTATUSID == 2 ? 'Lock' : 'Unlock'}</td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center align-items-center">
                                            ${employee.ACCOUNTSTATUSID == 2 ?
                                                        `<button type="button" class="btn btn-danger btn-unlock" data-empid="${employee.EMPLOYEEID}">
                                                    <i class="far fa-trash-can"></i> Unlock
                                                </button>` :
                                                        `<span class="text-success">Unlocked</span>`
                                                    }
                                        </div>
                        ...
                        `);
                                ...
                                error: function () {
                                    Swal.fire('ข้อผิดพลาด!',
                                        'ไม่สามารถค้นหาพนักงานได้ กรุณาลองอีกครั้ง.',
                                        'error');
                                }
                            });
                        });
                            ...
                            Swal.fire({
                                ...
                                if (result.isConfirmed) {
                                    $.ajax({
                                        ...
                                        error: function () {
                                            Swal.fire('ข้อผิดพลาด!',
                                                'ไม่สามารถลบพนักงานได้ กรุณาลองอีกครั้ง.',
                                                'error');
                                        }
                                    });
                                }
